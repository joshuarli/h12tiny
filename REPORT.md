...
| h12tiny-web | Router, extractors, response conversion, route limits/deadlines, optional JSON/query/SSE/CORS/raw upgrade/RFC 6455 framing | Pooling, TLS, WebSocket application message policy, Tower |
...
| websocket | Optional RFC 6455 HTTP/1.1 validation, switching response, and server-role futures-lite framing |
| json | util JSON and web JSON when web is present |
| query, sse, cors | Web-only optional application vocabulary |
| full | Client, server, util, web, both protocols, TLS, upgrade, WebSocket, JSON, query, SSE, and CORS |
...
- HyperIo<T> in core adapts Hyper upgraded I/O back to futures-I/O for raw upgrade consumers and h12tiny-web's optional WebSocket adapter.
...
| tests/websocket_interop.rs | The optional WebSocket extractor validates the RFC 6455 handshake, reads a masked text frame, and writes the echoed frame. |
...
| Application-specific | SSE keepalive policy and WebSocket session/PTY protocol | SSE data framing and the RFC 6455 transport boundary are supplied; keepalive scheduling and WebSocket session policy use application code. |
...
The extension still does not add Tokio, native TLS, Reqwest, Axum, Tower, proxy or redirect frameworks, OpenAPI, application tracing, HTTP/3, or HTTP/2 extended CONNECT. The optional WebSocket feature owns only RFC 6455 transport mechanics, not application sessions or message policy. HTTP/1 h2c upgrade remains omitted; cleartext H2 is prior knowledge.
