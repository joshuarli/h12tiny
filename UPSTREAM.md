...
| Dependency policy | `libc` is permitted directly or transitively; the normal-graph check rejects the remaining forbidden packages. |
...
| `src/client/connect.rs` | `src/client/legacy/connect/dns.rs`, `src/client/legacy/connect/http.rs` | rewritten | Direct `async-net` TCP plus `futures-rustls`/Rustls/ALPN transport; no proxy, socket2, or native TLS machinery. The implementation currently declares no direct `libc`, but direct or transitive `libc` is permitted by policy. |
...
| `tests/legacy_client.rs`: waiter cancellation and retry after failed connect | `src/client/pool.rs::dropped_checkout_removes_its_waiter`; `cancelled_h2_establishment_wakes_waiter_and_allows_retry`; `tests/lifecycle_regressions.rs::failed_h2_establishment_releases_multiple_waiters_for_later_success` | covered locally | The network regression proves one failed H2 owner, two released waiters on a replacement session, and a later successful checkout. |
...
| `tests/legacy_client.rs`: stale H1 connection retry | `tests/raw_h1.rs::closed_h1_session_is_not_reused_and_next_request_reconnects`; `tests/lifecycle_regressions.rs::stale_idle_h1_socket_is_evicted_after_failed_dispatch` | covered locally | A post-serialization stale dispatch is surfaced rather than unsafely replayed; the discarded session's next request reconnects successfully. |
